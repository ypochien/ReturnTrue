...
public class DynamicListView extends ListView {
	...
	public static interface OnScrollStatusListener {
		public void onScrollStatus(ScrollStatus position);
	}

	private int currentScrollState = OnScrollListener.SCROLL_STATE_IDLE;
	private ScrollStatus current = ScrollStatus.Idle;
	...
	public OnScrollStatusListener getListener() {
		return listener;
	}

	public void setListener(OnScrollStatusListener listener) {
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
		...
		public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
			if (e1 != null && e2 != null) {
				if (e2.getY() > e1.getY() && Math.abs(e2.getY() - e1.getY()) > dip2px(getContext(), 20)) {
					direction = MoveDirection.Up;
				} else if (e2.getY() < e1.getY() && Math.abs(e2.getY() - e1.getY()) > dip2px(getContext(), 20)) {
    ...
    private class MyOnScrollListener implements OnScrollListener {
        ...
        public void onScrollStateChanged(final AbsListView view, final int scrollState) {
    		...
    		invokeListener(ScrollStatus.Moving);
        }
	...
	private void invokeListener(ScrollStatus position) {
		if (listener != null && 
			current!= position) {
			listener.onScrollStatus(position);
		}
		current = position;
	}
